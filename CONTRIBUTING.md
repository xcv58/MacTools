...
- A manual GitHub Actions `Build` run uploads a 14-day `MacTools-Debug` development snapshot containing `MacTools Dev.app` and plugins built from the same commit. Maintainers may schedule that snapshot daily at `06:00 UTC` with the repository variable `ENABLE_NIGHTLY_BUILDS=true`; see `docs/github-actions.md` for activation, download, and pause instructions. This Debug artifact is not the public, auto-updating MacTools Nightly channel: it is not Developer ID signed or notarized, does not create a GitHub Release or Sparkle appcast, and cannot update in-app. The final Nightly channel is tracked in [issue #243](https://github.com/ggbond268/MacTools/issues/243).
