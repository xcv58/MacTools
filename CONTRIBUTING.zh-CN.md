...
- 手动运行 GitHub Actions `Build` 会上传保留 14 天的 `MacTools-Debug` 开发快照，其中包含同一 commit 构建的 `MacTools Dev.app` 和插件。维护者可通过仓库变量 `ENABLE_SCHEDULED_DEV_SNAPSHOTS=true` 让该快照每天 `06:00 UTC` 生成；启用、下载与暂停步骤见 `docs/github-actions.md`。这个 Debug artifact 不是面向公众、可在应用内更新的 MacTools Nightly：它没有 Developer ID 签名或 Apple 公证，不会创建 GitHub Release 或 Sparkle appcast，也不能在应用内更新。最终 Nightly 渠道由 [issue #243](https://github.com/ggbond268/MacTools/issues/243) 跟踪，并且必须使用独立的 `ENABLE_NIGHTLY_RELEASES` 发布开关。
